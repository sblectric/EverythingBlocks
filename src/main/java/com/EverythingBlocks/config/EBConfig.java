...
public class EBConfig {
	...
	public static void loadConfig(File f) {
		...
		p.setComment("Set to false to hide ALL Everything Block variants from the creative tab / NEI / etc.");
		enableCreativeTabVariants = p.getBoolean();
		
		// Show slabs
		p = config.get("General", "Enable creative tab slabs", showSlabs);
		p.setComment("Set to false to hide Everything Slab variants from the creative tab / NEI / etc.");
		showSlabs = p.getBoolean();
		
		// Show stairs
		p = config.get("General", "Enable creative tab stairs", showStairs);
		p.setComment("Set to false to hide Everything Stairs variants from the creative tab / NEI / etc.");
		showStairs = p.getBoolean();
		
		// Show walls
		p = config.get("General", "Enable creative tab walls", showWalls);
		p.setComment("Set to false to hide Everything Wall variants from the creative tab / NEI / etc.");
		...
		p.setComment("Mods by mod id that should not be shown in the Everything Blocks creative tab / NEI / etc.");
		blacklistedMods = new JointList<String>().join(p.getStringList());
		
		// 2x2 recipe settings
		p = config.get("Crafting", "Ignore 2x2 recipes", ignore2x2Recipes);
		p.setComment("Set to true to be able to craft Everything Blocks made from items that have existing 2x2 crafting recipes.");
