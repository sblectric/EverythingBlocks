...
public class EverythingWallCraftingRecipes implements IRecipe {
	...
	private boolean isValidRecipeGrid(InventoryCrafting inv) {
		...
		if(Block.getBlockFromItem(s.get(0).getItem()) == EBBlocks.blockEverything) {
			if(EBUtils.areItemStacksEqualForCrafting(inv.getStackInSlot(0), inv.getStackInSlot(1), inv.getStackInSlot(2), inv.getStackInSlot(3), 
					inv.getStackInSlot(4), inv.getStackInSlot(5))) {
				return true;
			}
			if(EBUtils.areItemStacksEqualForCrafting(inv.getStackInSlot(3), inv.getStackInSlot(4), inv.getStackInSlot(5), inv.getStackInSlot(6), 
