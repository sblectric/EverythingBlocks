...
public class EBInventoryCrafting extends InventoryCrafting {
	
	protected ItemStack[] stackList;

	private EBInventoryCrafting(Container c, int xSz, int ySz) {
		super(c, xSz, ySz);
        int k = xSz * ySz;
        this.stackList = new ItemStack[k];
	}
	
	private EBInventoryCrafting(int xSz, int ySz) {
		this(null, xSz, ySz);
	}
	
	public EBInventoryCrafting(int xSz, int ySz, List<ItemStack> contents) {
		this(xSz, ySz);
		int a = 0;
